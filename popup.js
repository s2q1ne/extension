...
let myTabTitle = document.getElementById('myTabTitle');
let myTab = document.getElementById('myTabContainer');
...
function tabDataUpdateAndTabElementUpdate(){
  chrome.storage.sync.get('tabData', function(data){
    data_tabs=data.tabData;
    tabElementUpdate();
  })
...
saveUrlElement.onclick = function(element) {
  ...
  currentWindow: true
    ...
    hash = sha3_512.create();
    chrome.storage.sync.get('codes', function(codes){
      codes['[new code]']
    })
